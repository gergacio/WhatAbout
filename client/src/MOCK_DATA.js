export const Data = [
  {
    "question": "Internet",
    "answer": [" Global system of interconnected computer networks that uses the Internet protocol suite (TCP/IP) to communicate between networks and devices."],
    "path": ""
  },
  {
    "question": "WWW",
    "answer": [" Information system enabling information to be shared over the Internet."],
    "path": ""
  },
  {
    "question": "JavaScript",
    "answer": [" Programming language that is one of the core technologies of the World Wide Web."],
    "path": ""
  },
  {
    "question": "Values",
    "answer": [" Values represent task we want to accomplish.", "Values have an behavior associated with them."],
    "path": require("./images/values.png"),  
  },
  {
    "question": "Operations",
    "answer": [" Operation is what we do with our values."],
    "path": require("./images/operations.png"),
  },
  {
    "question": "Types",
    "answer": [" Type is referring with kind of the value."],
    "path": require("./images/types.png")
  },
  {
    "question": "Variables",
    "answer": [" Variable is a representation in some place in memory where we store data."],
    "path": require("./images/types.png")
  },
  {
    "question": "Expressions",
    "answer": [" Expression is part of entire thing."],
    "path": require("./images/expressions.png")
  },
  {
    "question": "Statements",
    "answer": [" Statement is an entire thing finish with semicolon."],
    "path": require("./images/statements.png")
  },
  {
    "question": "Conditions",
    "answer": [" Conditions refer to decision making. if statement is the primary way to make a decision."],
    "path": require("./images/conditions.png")
  },
  {
    "question": "Loops",
    "answer": [" When have set of operation to do multiple times comes Loops"],
    "path": require("./images/loops.png")
  },
  {
    "question": "Functions",
    "answer": [" Function is a collection of statements which we want to do multiple time in multiple places."],
    "path": require("./images/functions.png")
  },
  {
    "question": "Primitive data",
    "answer": [" Primitive types: ..."],
    "path": ""
  },
  {
    "question": "NaN",
    "answer": [" NaN: ..." ],
    "path": ""
  },
  {
    "question": "new",
    "answer": [" new: ..."],
    "path": ""
  },
  {
    "question": "Coersion",
    "answer": [" Coersion: ..."],
    "path": ""
  },
  {
    "question": "Booleans",
    "answer": [" Booleans: ..."],
    "path": ""
  },
  {
    "question": "Equality",
    "answer": [" Equality: ..."],
    "path": ""
  },
  {
    "question": "Scope",
    "answer": [" Scope: ..."],
    "path": ""
  },
  {
    "question": "Undefined",
    "answer": [" Undefined: ..."],
    "path": ""
  },
  {
    "question": "Undeclared",
    "answer": [" Undeclared: ..."],
    "path": ""
  },
  {
    "question": "Function Expressions",
    "answer": [" Function Expressions: ..."],
    "path": ""
  },
  {
    "question": "IIFE",
    "answer": [" IIFE: ..."],
    "path": ""
  },
  {
    "question": "Block Scoping",
    "answer": [" Block Scoping: ..."],
    "path": ""
  },
  {
    "question": "Closure",
    "answer": [" Closure: ..."],
    "path": ""
  },
  {
    "question": "this",
    "answer": [" this: ..."],
    "path": ""
  },
  // {
  //   "question": "Prototypes",
  //   "answer": [" Prototypes: ..."],
  //   "path": ""
  // },
  {
    "question": "Class",
    "answer": [" Class: ..."],
    "path": ""
  }
]